...
def _get_item(dynamodb_resource):
    ...
    return table.get_item(Key={"user_id": USER_ID, "file_id": FILE_ID}).get("Item")


class TestCreateRecord:
    def test_creates_item_with_correct_keys(self, tracker, dynamodb_resource):
        tracker.create_record(USER_ID, UPLOAD_ID, FILENAME, LANDING_BUCKET, LANDING_KEY)
...
class TestStartStage:
    def test_appends_stage_entry(self, tracker, dynamodb_resource):
        tracker.create_record(USER_ID, UPLOAD_ID, FILENAME, LANDING_BUCKET, LANDING_KEY)
        tracker.start_stage(
            USER_ID, UPLOAD_ID, FILENAME, UploadStage.ROUTING, LANDING_BUCKET, LANDING_KEY
        )
        item = _get_item(dynamodb_resource)
    ...
    def test_multiple_stages_appended_in_order(self, tracker, dynamodb_resource):
        ...
        tracker.complete_stage(
            USER_ID, UPLOAD_ID, FILENAME, UploadStage.ROUTING, RAW_BUCKET, RAW_KEY
        )
        tracker.start_stage(
            USER_ID, UPLOAD_ID, FILENAME, UploadStage.ENRICHMENT, RAW_BUCKET, RAW_KEY
        )
        item = _get_item(dynamodb_resource)
...
class TestFailStage:
    def test_updates_status_to_failed(self, tracker, dynamodb_resource):
        ...
        tracker.fail_stage(USER_ID, UPLOAD_ID, FILENAME, UploadStage.ROUTING, "Copy failed")
        item = _get_item(dynamodb_resource)
